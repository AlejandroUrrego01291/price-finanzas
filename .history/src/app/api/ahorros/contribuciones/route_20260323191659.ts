...
export async function POST(request: Request) {
    try {
        ...
        console.log('=== DETALLE DE CONTRIBUCIONES ===')
        console.log('Meta:', saving.concept)
        console.log('Número de contribuciones existentes:', saving.contributions.length)

        // Mostrar cada contribución para debug
        saving.contributions.forEach((c, idx) => {
            console.log(`  Contribución ${idx + 1}: amount=${c.amount}, extra=${c.extraAmount || 0}, total parcial=${c.amount + (c.extraAmount || 0)}`)
        })
