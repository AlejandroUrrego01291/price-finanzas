...
export async function POST(request: Request) {
    try {
        ...
        const fechaTransaccion = new Date(date)
        const hoy = new Date()
        hoy.setHours(0, 0, 0, 0)

        // Si la fecha es hoy o anterior, marcamos como completado, si es futura, no
...
export async function PUT(request: Request) {
    try {
        ...
        const { type, conceptId, conceptName, value, date, category, subType, completed } = body
...
// PATCH - Marcar transacción como completada (toggle)
export async function PATCH(request: Request) {
    try {
        ...
        const transaccion = await prisma.transaction.update({
            where: { id },
            data: { completed: completed !== undefined ? completed : !existing.completed },
            include: {
                concept: true
            }
        })
        ...
        console.error('Error en PATCH transaccion:', error)
        return NextResponse.json(
