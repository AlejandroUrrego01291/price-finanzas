    ...
    const handleRegisterContribution = async (savingId: string, contributionData: any) => {
        try {
            ...
            if (response.ok) {
                ...
                // Convertir fechas de las contribuciones
                ...
                // Actualizar estado local - USAR EL NUEVO VALOR DEL TOTAL AHORRADO
                const nuevoTotal = ahorroConvertido.contributions[0]?.totalSaved ?? 0

                // Actualizar el estado de ahorros y total ahorrado
                setAhorros(prev => prev.map(a => a.id === savingId ? ahorroConvertido : a))
                setTotalAhorrado(prev => {
                    // Calcular nuevo total sumando todos los ahorros actualizados
                    const nuevosAhorros = ahorros.map(a => a.id === savingId ? ahorroConvertido : a)
                    return nuevosAhorros.reduce((sum, a) => {
