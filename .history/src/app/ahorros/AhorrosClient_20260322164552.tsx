    ...
    const handleRegisterContribution = async (savingId: string, contributionData: any) => {
        try {
            ...
            if (response.ok) {
                ...
                // Convertir la fecha de la nueva contribución
                const nuevaContribucion = {
                    ...ahorroActualizado.contributions[0],
                    date: new Date(ahorroActualizado.contributions[0].date).toISOString().split('T')[0]
                }

                const nuevasContribuciones = [nuevaContribucion, ...(ahorros.find(a => a.id === savingId)?.contributions || [])]
                const nuevoAhorrado = ahorroActualizado.contributions[0]?.totalSaved ?? 0

                setAhorros(ahorros.map(a => a.id === savingId ? {
                    ...a,
                    contributions: nuevasContribuciones.slice(0, 10),
                    ahorrado: nuevoAhorrado,
                    restante: a.targetAmount - nuevoAhorrado
                } : a))

                const nuevoTotalAhorrado = ahorros.reduce((sum, a) => {
                    if (a.id === savingId) {
                        return sum + nuevoAhorrado
                    }
