    ...
    const handleRegisterContribution = async (savingId: string, contributionData: any) => {
        try {
            ...
            if (response.ok) {
                ...
                // Obtener el nuevo total ahorrado de la meta actualizada
                const nuevoAhorrado = ahorroConvertido.contributions[0]?.totalSaved ?? 0

                // Actualizar el estado de ahorros
                setAhorros(prev => prev.map(a => a.id === savingId ? ahorroConvertido : a))

                // Recalcular el total ahorrado sumando todas las metas
                setTotalAhorrado(prevTotal => {
                    // Calcular sumando todas las metas actualizadas
                    const nuevosAhorros = ahorros.map(a =>
                        a.id === savingId ? ahorroConvertido : a
                    )
