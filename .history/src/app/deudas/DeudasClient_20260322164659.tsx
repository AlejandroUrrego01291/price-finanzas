    ...
    const handleRegisterPayment = async (debtId: string, paymentData: any) => {
        try {
            ...
            if (response.ok) {
                ...
                // Convertir la fecha del nuevo pago
                const nuevoPago = {
                    ...deudaActualizada.payments[0],
                    date: new Date(deudaActualizada.payments[0].date).toISOString().split('T')[0]
                }

                const nuevosPagos = [nuevoPago, ...(deudas.find(d => d.id === debtId)?.payments || [])]
                const nuevoSaldoActual = deudaActualizada.payments[0]?.remainingBalance ?? deudaActualizada.initialAmount

                setDeudas(deudas.map(d => d.id === debtId ? {
                    ...d,
                    payments: nuevosPagos.slice(0, 10),
                    saldoActual: nuevoSaldoActual
                } : d))

                const nuevoTotalDeudas = deudas.reduce((sum, d) => {
                    if (d.id === debtId) {
                        return sum + nuevoSaldoActual
                    }
