    ...
    const handleRegisterPayment = async (debtId: string, paymentData: any) => {
        try {
            ...
            if (response.ok) {
                ...
                setDeudas(deudas.map(d => d.id === debtId ? deudaConvertida : d))

                // Recalcular total de deudas
                const nuevoTotal = deudas.reduce((sum, d) => {
                    if (d.id === debtId) {
                        const ultimoPago = deudaActualizada.payments[0]
                        return sum + (ultimoPago?.remainingBalance ?? deudaActualizada.initialAmount)
                    }
