    ...
    const handleRegisterPayment = async (debtId: string, paymentData: any) => {
        try {
            ...
            if (response.ok) {
                ...
                // Actualizar estado local - USAR EL NUEVO VALOR DEL SALDO ACTUAL
                setDeudas(prev => prev.map(d => d.id === debtId ? deudaConvertida : d))

                // Recalcular total de deudas
                setTotalDeudas(prev => {
                    const nuevasDeudas = deudas.map(d => d.id === debtId ? deudaConvertida : d)
                    return nuevasDeudas.reduce((sum, d) => {
