...
export default function DeudasClient({ deudas: deudasIniciales, totalDeudas: totalInicial }: Props) {
    ...
    const handleRegisterPayment = async (debtId: string, paymentData: any) => {
        try {
            ...
            if (response.ok) {
                // Actualizar la deuda con el nuevo pago
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between h-16">
                        <div className="flex items-center">
                            <h1 className="text-xl font-semibold text-gray-900">Price - Administración de Deudas</h1>
                        </div>
                    ...
                    {deudas.map((deuda) => {
                        ...
                        return (
                            <div key={deuda.id} className="bg-white shadow rounded-lg overflow-hidden">
                                    ...
                                    <div className="flex justify-between items-start">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-600">
                                                Fecha de inicio: {formatearFecha(deuda.startDate)}
                                            </p>
                                        </div>
                                ...
                                <div className="px-6 py-4 grid grid-cols-1 md:grid-cols-3 gap-4 bg-white">
                                    <div>
                                                ...
                                                <tbody className="bg-white divide-y divide-gray-200">
                                                    {deuda.payments.map((pago) => (
                                                        <tr key={pago.id} className="hover:bg-gray-50">
                                                            <td className="px-6 py-2 whitespace-nowrap text-sm">{formatearFecha(pago.date)}</td>
                                                            <td className="px-6 py-2 whitespace-nowrap text-sm">{formatearMoneda(pago.interestPaid)}</td>
                                                            <td className="px-6 py-2 whitespace-nowrap text-sm">{formatearMoneda(pago.capitalPaid)}</td>
                                                            <td className="px-6 py-2 whitespace-nowrap text-sm">{pago.extraPayment ? formatearMoneda(pago.extraPayment) : '-'}</td>
                                                            <td className="px-6 py-2 whitespace-nowrap text-sm font-medium">{formatearMoneda(pago.paidAmount)}</td>
                                                            <td className="px-6 py-2 whitespace-nowrap text-sm">{formatearMoneda(pago.remainingBalance)}</td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
